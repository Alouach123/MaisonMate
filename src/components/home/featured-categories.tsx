      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-6">
          {categoriesToDisplay.map((category) => {
            ...
            const defaultIconBgClass = 'bg-slate-100 dark:bg-slate-800';
            const defaultIconColorClass = 'text-slate-500 dark:text-slate-400';
            const defaultCardWrapperShadow = 'shadow-sm';

            // Hover state classes (using category.themeColor)
            // These utility classes need to be fully generated by Tailwind JIT
            // e.g., hover:border-green-500, hover:shadow-green-500/40, group-hover:bg-green-500, group-hover:text-green-700 etc.
            // Make sure your tailwind.config.ts safelist includes these if they are dynamically generated and not picked up.
            // However, for direct application like this, string interpolation should work.
            ...
            const iconHoverClasses = `group-hover:text-white dark:group-hover:text-${category.themeColor}-50`; // Adjusted for better contrast on dark themed icon bg
            ...
            return (
              <Link href={category.href} key={category.name} legacyBehavior>
                <a className={`group block p-0.5 rounded-xl ${defaultCardWrapperShadow} transition-all duration-300 ease-in-out 
                             ...
                             ${cardHoverClasses}
                           `}>
                  <Card className="h-full text-center border-0 shadow-none bg-card rounded-[10px]"> {/* bg-card makes it "white" or theme card color */}
                    <CardContent className="flex flex-col items-center justify-center p-3 md:p-4">
                      <div className={`p-3 md:p-4 rounded-full mb-3 
                                     ${defaultIconBgClass} 
                                     ${iconContainerHoverClasses} 
